...
namespace Warp::Runtime::Compiler
{
	...
	template<
		...
		>
	constexpr auto make_alternative_prototype_with_no_parameters(IdentifierParameterType name) noexcept
	{
		using AlternativePrototypeType = AlternativePrototype<
				SingleParameterType, 
				IdentifierParameterType
			>;
		return std::make_unique<AlternativePrototypeType>(std::move(AlternativePrototypeType{
				name, 
				std::vector<SingleParameterType>{}
			}));
	}
	template<typename IdentifierParameterType = std::string>
	constexpr auto make_alternative_prototype(IdentifierParameterType name, auto... parameters) noexcept
	{
		using SingleParameterType = CleanType<typename TakeFirstType<decltype(parameters)...>::Type>;
		std::vector<SingleParameterType> parameters_;
		(parameters_.push_back(std::move(parameters)), ...);
		auto prototype = AlternativePrototype{
					name, 
					std::move(parameters_)
				};
		return std::make_unique<decltype(prototype)>(std::move(prototype));
	}
}
