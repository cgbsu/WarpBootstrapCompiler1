...
//		>(context.value(), context.value().constants.at(name).value).value();
//}
//
//template<auto TypeTagParameterConstant>
...
//	//	>({"LetterOfTheDay"}, std::vector{'A'}, debug);
//};
TEST(FunctionDeclarations, TEST) {
	std::cout << FunctionDeclaritionTermsType::get_term_precedence<NumericLiteral::CharacterMark>() << "\n";
	std::cout << FunctionDeclaritionTermsType::get_term_precedence<Identifier::Identifier>() << "\n";
}
