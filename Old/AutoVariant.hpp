...
///////////////////////////////////////////////////////////////////////////////////
// This header was taken from WarpBootstrapCompiler0, and modified for the ////////
// purposes of WarpBootstrapCompiler1, written in C++23, since C++23 //////////////
// std::unique_ptr has become more "constexpr friendly", however this class ///////
// is still nessisary to handle incomplete types and so is NotSoUniquePointer /////
// No memory leaks have been detected by CppUTest and additiona facilities are ////
// being added to add proper deleters. ////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////
...
namespace Warp::Utilities
{
    ...
    struct AutoVariant
    {
        ...
        template<IsCompleteConcept AlternativeParameterType, typename... InitializersParameterTypes>
        constexpr AutoVariant(std::in_place_type_t<AlternativeParameterType>, 
        ...
        {
			visit<[](auto* data_, auto self)
				{
					using AlternativeType = std::remove_reference_t<
							std::remove_pointer_t<std::decay_t<decltype(data_)>>
						>;
					if constexpr(is_complete<AlternativeType> == true)
					{
						if(self->deleter.has_value() == false)
						{
							auto pointer = static_cast<decltype(data_)>(data_);
							//pointer->~AlternativeType();
							delete pointer;
						}
						else
							self->deleter.value()(data_);
        	        	return nullptr; 
					}
					else {
						self->deleter.value()(data_);
        ...
        protected: 
			...
			std::optional<DeleterType> deleter;
    };


    template<typename AlternativeType>
    constexpr bool holds_alternative(const auto& variant) {
        return std::remove_reference_t<std::decay_t<decltype(variant)>>
				::template type_index<AlternativeType> == variant.index();
    }
       ...
       >
    {
        ...
        constexpr VisitImplementation(
			...
			: result(VisitorParameterConstant(
					static_cast<PointerType>(variant.get_data()), 
					additional_arguments...
				)) {}
    };
